...
- Header、快速添加、URL 捕获、PWA 快捷入口、分享目标、命令面板、菜单、语言菜单、自定义下拉、视图切换、任务卡片、日历、统计、弹窗、toast 和存储不可用状态。
...
- URL 捕获和分享目标会聚焦快速添加或复用快速添加保存路径，并通过 live region 播报预填状态；分享目标确认保存后会保留正文和 URL 描述。
...
- 跑完上述四条自动化命令。
- 用键盘完成快速添加、URL 捕获预填、命令面板、新增、编辑、完成、归档、批量操作、导入、导出/备份、筛选、排序、日历详情和统计跳转。
