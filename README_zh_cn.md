...
Task Tracer 是一个面向个人使用的任务工作台。它运行在浏览器中，任务数据保存在当前设备/浏览器配置内，适合日常任务捕获、项目跟进、截止日期工作、重复事项、本地提醒、任务归档和轻量效率回顾，不需要账号或云同步。

## 功能亮点

| 范围 | Task Tracer 提供 |
| --- | --- |
| 记录 | 完整任务弹窗、快速添加解析、截止日期和时间、无日期任务、描述、子任务、项目和标签。 |
| 组织 | 项目分组、多标签、智能搜索、状态筛选、保存智能视图、手动排序和归档历史。 |
| 规划 | 列表、日历、时间线和统计视图，配合跨时区稳定的截止日期与清晰状态片段。 |
| 执行 | 顶部快捷入口包含今日计划、批量操作、撤销、新建任务、通知、主题和应用菜单。 |
| 例行事项 | 每天、每周、指定星期、每月、每月最后一天和自定义间隔重复。 |
| 提醒 | 提前提醒、重复提醒、稍后提醒、错过提醒提示，以及明确的浏览器提醒限制说明。 |
| 回顾 | 完成率、进行中任务逾期率、归档数量、今日完成数、连续完成天数和近期趋势。 |
| 数据安全 | 导出/备份、备份健康、导入预览、合并/替换冲突处理、完整性校验、替换前快照、撤销，以及存储受限时的紧急备份。 |
| 使用体验 | 响应式布局、明暗主题、减少动态效果、键盘导航、读屏标签和中英文界面。 |

## 日常流程

1. 使用完整弹窗添加任务，或用快速添加输入一条轻量任务。
2. 用项目做主要分组，用标签连接跨项目上下文。
3. 把常用搜索、筛选和视图保存为智能视图。
4. 在列表视图中完成、编辑、稍后提醒、处理子任务、归档、批量操作和手动排序。
5. 把当前要做的任务加入今日计划，需要执行时使用顶部今日计划按钮进入专注状态。
6. 当日期排布比列表顺序更重要时，切换到日历视图或时间线视图。
7. 用统计视图查看当前负载、逾期压力、归档历史和近期完成趋势。
8. 在清理浏览器数据、更换设备、测试导入或重新安装前，先导出/备份 JSON 文件。
...
项目是主要分组。一个任务属于一个项目，例如 `工作`、`个人` 或某个具体计划。项目选择器可以收窄任务空间，全部项目列表会按项目分组。

标签是灵活的辅助标记。一个任务可以有多个标签，例如 `学习`、`设计`、`复盘`，适合连接不同项目中的同类任务。
...
| 视图 | 适合做什么 |
| --- | --- |
| 列表 | 日常执行任务，包含完整操作、子任务、提醒、状态片段、进度条、归档控制和手动排序。 |
| 日历 | 按真实月历查看日期安排，支持上一月/下一月、上一年/下一年、回到今天、日期详情和无日期分组。 |
| 时间线 | 按日期顺序浏览即将到来或历史阶段的任务。 |
| 统计 | 查看完成率、进行中任务逾期率、归档数量、今日完成数、连续完成天数和近期趋势。 |
...
| 动作 | 用途 |
| --- | --- |
| 导出/备份 | 下载带导出 ID、完整性校验和版本说明的完整任务 JSON，并更新最近备份时间。 |
| 导入 | 写入前先预览 JSON 文件，包含恢复检查、任务数量、导入差异、重复 ID、同名重复项、当前数据影响和同名冲突处理选项。 |

恢复检查清单会识别 Task Tracer 备份元数据、模板兼容状态、完整性校验状态、任务内容、重复 ID、本地同名项和替换当前数据的影响。替换导入会在覆盖现有任务前下载导入前快照。完整性校验失败时，替换导入需要显式风险确认；合并模式仍可用于更稳妥地恢复数据。

备份健康状态会显示在应用菜单中，用来提示今天已备份、最近已备份，或建议重新备份。

## 安装、离线与更新

Task Tracer 是 PWA：当浏览器支持这一标准时，它可以像应用一样安装和打开。应用菜单中提供安装入口。如果当前浏览器没有自动安装弹窗，同一个入口会显示桌面 Chrome/Edge、Android Chrome 和 iOS Safari 的手动安装方式。
