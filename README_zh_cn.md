...
Task Tracer 是一个基于截止日期的任务管理 PWA，适合个人计划、学习安排和周期性事项跟踪。它把任务数据保存在浏览器本地，加载后支持离线使用，并围绕截止日期状态、提醒、项目分组、标签、子任务、归档、导入安全和本地备份组织体验。
...
- 备份：下载带版本说明的本地快照，记录最近备份时间，用于降低浏览器数据被清理后的丢失风险。
